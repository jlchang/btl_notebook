...
if [ "$1" = "miseq" ]
  then
    UGERPARAM1="-cwd -l h_vmem=5G -l h_rt=4:00:00"
fi
...
if [ "$1" = "hiseq" ]
  then
    UGERPARAM1="-cwd -l h_vmem=15G -l h_rt=18:00:00"
fi
...
do
  ...
  echo "job to run in analysis dir: qsub $UGERPARAM1 $UGERPARAM2 $SCRIPTDIR/pairedAlignMarkD.sh ${sample} ${fastq1} ${fastq2}"
  qsub $UGERPARAM1 $UGERPARAM2 $SCRIPTDIR/pairedAlignMarkD.sh ${sample} ${fastq1} ${fastq2}
done < input_data.tsv
#done < test.in
