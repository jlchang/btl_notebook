...
do
  ...
  mkdir ${sample}_PE
  cd ${sample}_PE
  UGERPARAMS="-cwd -l h_vmem=20G -l h_rt=12:00:00 -N S_${sample}"
    #syntax to use to avoid bad UGER host(s))
    #UGERPARAMS="-cwd -l h_vmem=10G -l h_rt=4:00:00 -l h=\'!(uger-c075|uger-c088)\' -N S_${sample}"
  echo "job to run in analysis dir: qsub $UGERPARAMS $SCRIPTDIR/ratPairedAlignMarkD.sh ${sample} ${fastq1} ${fastq2}"
