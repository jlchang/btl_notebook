...
SCRIPTDIR="/cil/shed/sandboxes/jlchang/notebook/scripts/ChIPseq/paired/v0.04/rat"
...
do
  ...
  echo "job to run in analysis dir: qsub $UGESPARAMS $SCRIPTDIR/pairedAlignMarkD.sh ${sample} ${fastq1} ${fastq2}"
  qsub $UGESPARAMS $SCRIPTDIR/ratPairedAlignMarkD.sh ${sample} ${fastq1} ${fastq2}
done < input_data.tsv
#done < test.in
