...
#MAPQ Reads
MR=$(head -1 ${sample}.mapq28.PE.nodup.flagstat | cut -d " " -f 1)
...
mperip=$(cat ${sample}.mapq28.PE.nodup.H3K27ac.rip)
...
echo -e "$TR\t$PR\t$perip\t$pefrip\t$MR\t$mperip\t$mpefrip\t$AR\t$aqrip\t$aqfrip\t$R1T\t$R1C\t$R1A\t$R2T\t$R2C\t$R2A\t$elcpr\t$dup\t$els\t$MRR\t$PIS\t$PP\t$sing\t$adup\t$mapq0\t$mapq5\t$mapq10\t$mapq28\t$mapq29\t$mmdc"
