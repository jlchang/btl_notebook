...
NSC=$(cut -f 9 ${sample}.mapq1.PE.nodup.15M.tagAlign.gz.qc)
RSC=$(cut -f 10 ${sample}.mapq1.PE.nodup.15M.tagAlign.gz.qc)
QT=$(cut -f 11 ${sample}.mapq1.PE.nodup.15M.tagAlign.gz.qc)
...
echo -e "Tot_Reads\tMAPQ1_Reads\tmapq1PE_rip\tmapq1PE_FRIP\tR1_CHIMERAS\tR1_ADAPTER\tR2_CHIMERAS\tR2_ADAPTER\tpDUPLICATION\tEstLibSize\tRSC\tNSC\tQT\tMapRaw_Reads\tPropPr_Reads\tPrSing_Reads\tmmdc_Reads\tpDup_Reads"
echo -e "$TR\t$MR\t$mperip\t$mpefrip\t$R1C\t$R1A\t$R2C\t$R2A\t$dup\t$els\t$RSC\t$NSC\t$QT\t$MRR\t$PP\t$sing\t$mmdc\t$pdup"
